...
def main():
    ...
    with open('my_slam/intrinsicNew.npy', 'rb') as f:
        intrinsic = np.load(f)
    ...
    while video.isOpened(): #заходим в цикл управления 
        ...
        if ret:
            ...
            """
            ------------------------------------------
            ЗДЕСЬ РЕАЛИЗУЕТСЯ УПРАВЛЕНИЕ РОБОТОМ
            ------------------------------------------

            Control.control_robot(central_line, frame)
            ...
            """
            ------------------------------------------
            ЗДЕСЬ РЕАЛИЗУЕТСЯ ОТРИСОВКА ТРАЕКТОРИИ
            ------------------------------------------
            """
