...
class Lane:
  ...
  """
  def __init__(self, orig_frame, ROI):
    """
      Default constructor
    ...
    self.roi_points = np.float32([
      ROI[0], # Top-left corner
      ROI[1], # Bottom-left corner            
      ROI[2], # Bottom-right corner
      ROI[3] # Top-right corner
    ])
  ...
  def overlay_lane_lines(self, plot=False):
    ...
    midpoint = (self.left_fitx[-1] + self.right_fitx[-1]) // 2
    # Calculate the y-coordinates for the line
    y_bottom = self.orig_frame.shape[0] - 1
    y_top = int(y_bottom * 0.6)  # You can adjust the position of the top of the line
    # Define the points for the center line
    center_line_pts = np.array([[midpoint, y_bottom], [midpoint, y_top]], dtype=np.int32)
    self.center_line = center_line_pts
    # Draw the center line
    cv2.polylines(color_warp, [center_line_pts], isClosed=False, color=(255, 0, 0), thickness=5)
